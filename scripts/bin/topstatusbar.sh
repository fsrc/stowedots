...
BIGBAR_W=65
WIDTH_L=510
WIDTH_R=856 #WIDTH_L + WIDTH_R = 1366
HEIGHT=16
X_POS_L=0
X_POS_R=$WIDTH_L
Y_POS=0
...
printCPUInfo() {
  CPUPerf=$(tail -n19 /proc/cpuinfo|grep "cpu MHz"|awk '{ print $4 }'|awk -F. '{print $1}')
	# [[ $CPUPerf -gt 70 ]] && CPUPerf="^fg($CRIT)$CPUPerf^fg()"
	...
	echo -n "^fg($BAR_FG)${CPULoad0}%^fg($DZEN_FG2)/^fg($BAR_FG)${CPULoad1}%^fg($DZEN_FG2)/^fg($BAR_FG)${CPULoad2}%^fg($DZEN_FG2)/^fg($BAR_FG)${CPULoad3}% ^fg($DZEN_FG2)@ ^fg($BAR_FG)${CPUPerf}"
	return
}
...
printTempInfo() {
	CPUTemp0=$(sensors coretemp-isa-0 | grep "Core 0:" | awk '{print substr($3,2,2)}')
	CPUTemp1=$(sensors coretemp-isa-0 | grep "Core 1:" | awk '{print substr($3,2,2)}')
...
printLangInfo() {
  LANG=$(currlang.sh)
  echo -n "^fg($DZEN_FG2)KEYB ^fg($BAR_FG)${LANG}"
}
